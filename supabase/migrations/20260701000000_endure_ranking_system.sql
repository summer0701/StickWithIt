create table if not exists public.exercise_scores (
  ...
  exercise_type text not null check (exercise_type in ('running', 'squat', 'plank', 'pushup', 'extra')),
...
create table if not exists public.user_endure_ratings (
  ...
  plank_score integer not null default 0 check (plank_score between 0 and 1000),
...
create table if not exists public.ghost_profiles (
  ...
  plank_score integer not null check (plank_score between 0 and 1000),
